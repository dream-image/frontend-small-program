<template>
	...
	<view
		style="position: absolute;width: 100%;height: 100%;display: grid;background-image:linear-gradient(180deg,#fbfbfb,#faf0e4)">
		...
		<view style="grid-row-start: 3;grid-row-end: 16	;position: relative;position: relative;" class="items"
			v-if="questionList.length!=0 && !isNoErrorQuestion">

			<view
				style="border-radius: 15rpx;border-color:#ffa000 ;height: max-content;position: absolute;width: 100%;">
				<van-button round color="#f26944" style="position: absolute; left: 50rpx;top: 40rpx;" size="mini">
			...
			<view
				...
				class="items">
				<button class="mini-btn" plain="true" size="mini" style="height: 73rpx;padding: 10rpx 10rpx;
				width: 73rpx;margin-right: 20rpx;background-color: #ffcd29;color: white;
				" @click="questionIndex!=0?questionIndex--:$refs.firstQuestion.open()"><van-icon name="arrow-left" /></button>

				<van-button round color="#ffc400" @click="star" style="margin-right: 10rpx;">
					<van-icon name="star" :color="questionList[questionIndex].isStar==true?'#f57f17':'white'" />
				</van-button>

				<van-button round color="#ffc400" @click="questionList[questionIndex].isShowAnswer=true">不会</van-button>

				<button class="mini-btn" size="mini" plain="true" style="height: 73rpx;padding: 10rpx 10rpx;
				width: 73rpx;margin-left: 20rpx;background-color: #ffcd29;color: white;"
...
<style>
	...
	.correctButton {
		background-image: linear-gradient(150deg, #b4fad3, #48fa97);
	}

	.errorButton {
		background-image: linear-gradient(150deg, #ffbaba, #ea5455);
	}
	.chosenButton{
