<template>
	...
	<view style="position: absolute;width: 100%;height: 100%;background-image:linear-gradient(180deg,#fbfbfb,#faf0e4);">
		...
		<view style="display: flex;margin-top: 30rpx;margin-bottom: 30rpx;">
			<view
				style="display: flex;width: 70%;height: 70rpx;border: 1px solid #29b6f6;border-radius: 9999rpx;position: relative;margin-left: 60rpx;">
				<van-icon name="search" color="#1989fa" style="margin-left: 20rpx;" />
				<input type="text" v-model.lazy="searchText"
					style="padding: 10rpx;position: relative;top: 6rpx;font-family: '宋体';" placeholder="搜索" />
			</view>
			<van-button plain color="#f9a825" style="margin-left: 10rpx;" round @click="search">搜索</van-button>
		</view>

		<view v-if="realShowList.length!=0">
			<view v-for="i in realShowList" :key="i.id" style="position: relative;">
				<view
					style="border-radius: 15rpx;border-color:#ffa000 ;height: max-content;margin-top: 20rpx;position: relative;">
					<van-button round color="#f9a825" style="position: absolute; left: 10rpx" size="mini">
						{{ i.type }}
					</van-button>
